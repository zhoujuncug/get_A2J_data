...
def dataPreprocess(index, img, keypointsUVD, center, mean, std, lefttop_pixel, rightbottom_pixel, xy_thres=90, depth_thres=75, augment=True):
    ...
    imgResize[np.where(imgResize >= center[index][0][2] + depth_thres)] = depth_thres# center[index][0][2] 
    imgResize[np.where(imgResize <= center[index][0][2] - depth_thres)] = -1 * depth_thres# center[index][0][2] 
