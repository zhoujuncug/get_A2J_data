...
from lib.model.A2J import resnet
...
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv4(out)
        out = self.bn4(out)
        out = self.act4(out)  # (64, 256, 11, 11)
        out = self.output(out)  # (64, 448, 11, 11)

        # out is B x C x W x H, with C = 3*num_anchors
        out1 = out.permute(0, 3, 2, 1)
        batch_size, width, height, channels = out1.shape
        out2 = out1.view(batch_size, width, height, self.num_anchors, self.num_classes, 2)
        return out2.contiguous().view(out2.shape[0], -1, self.num_classes, 2)  # (64, 1936, 14, 2)  
...
class ClassificationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn4(out)
        out = self.act4(out)  # (64, 256, 11, 11)
        out = self.output(out)  # (64, 224, 11, 11)

        # out is B x C x W x H, with C = n_classes + n_anchors
        out1 = out.permute(0, 3, 2, 1)  # (64, 11, 11, 224)
        batch_size, width, height, channels = out1.shape
        out2 = out1.view(batch_size, width, height, self.num_anchors, self.num_classes)  # (64, 11, 11, 16, 14)
        return out2.contiguous().view(x.shape[0], -1, self.num_classes)  # (64, 1936, 14, 2)  
...
class ResNetBackBone(nn.Module):
    ...
    def forward(self, x): 
        ...
        x = x[:,0:1,:,:]  # depth
        x = x.expand(n,3,h,w)  # (64, 3, 176, 176)
        x = self.model.conv1(x)  # (64, 64, 88, 88)
        x = self.model.bn1(x)
        x = self.model.relu(x)
        x = self.model.maxpool(x)  # (64, 256, 44, 44)
        x1 = self.model.layer1(x)  # (64, 512, 44, 44)
        x2 = self.model.layer2(x1)  # (64, 512, 22, 22)
...
class A2J_model(nn.Module):
    ...
    def forward(self, x, prec=False): 
        ...
        regression = self.regressionModel(x4)  # (64, 1936, 14, 2)  
